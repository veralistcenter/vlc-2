<template>
	<main class="page pt--7">
		...
		<template v-if="single.networkInformation.externalLink.networkRelatedPages !== true">
			<MatrixBody class="mb--4" v-if="single.pageFormat.pageLength && $CheckA(single.pageFormat.body)" :matrix="single.pageFormat.body" />
			<section 
				v-if="!single.pageFormat.pageLength" 
				class="short_body richtext fs--regular" 
				v-html="single.pageFormat.shortBody"></section>
		</template>
	...
	export default{
		...
		mounted(){
			if(this.single.networkInformation.externalLink.makeExternalLink === true){
				this.$nextTick(() => {
					window.location.replace(this.single.networkInformation.externalLink.externalLinkUrl)
				})
			}
		},
