<template>
  <main class="page pt--8">
    <h1 class="genath section_heading title mb--1">
      ...
      <span v-html="b.biennialInfo.dateRange.startingYear"></span>–<span
        v-html="b.biennialInfo.dateRange.endingYear"
      ></span>
    </h1>
    ...
    <section class="grid">
      <aside class="col col--1_4 mcol--full">
        <figure>
          ...
          <figcaption
            ...
            v-if="$Check(b.featImage.imageCaption)"
            v-html="b.featImage.imageCaption"
          ></figcaption>
          <figcaption
            class="fs--small mt--1_4"
            v-else-if="
              $Check(b.featImage) && $Check(b.featImage.featuredImage.caption)
            "
            v-html="b.featImage.featuredImage.caption"
          ></figcaption>
        </figure>
      </aside>
      <section class="col col--1_2 mcol--full mmt--1">
        <section
          class="fs--regular"
          v-html="b.biennialInfo.fullDescription"
        ></section>
...
export default {
  head() {
    ...
    const image =
      this.$Check(this.b.featImage) &&
      this.$Check(this.b.featImage.featuredImage)
        ? this.b.featImage.featuredImage.sourceUrl
        : undefined;
  ...
  async asyncData({ $axios, $Req, store, params }) {
    ...
    try {
      ...
      const title = res.data.data.biennial
        ? res.data.data.biennial.title
        : "Focus Theme";
