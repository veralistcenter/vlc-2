<template>
  <main class="page pt--7">
    ...
    <header class="section_heading mb--4" v-else>
      ...
      <h3
        class="genath title"
        v-html="$Dated({ start: announcement.pageInfo.date, end: null })"
      ></h3>
      <p
        class="mt--1_2 max--500"
        v-if="
          $Check(announcement.pageInfo) &&
          $Check(announcement.pageInfo.previewInfo) &&
          $Check(announcement.pageInfo.previewInfo.description)
        "
        v-html="announcement.pageInfo.previewInfo.description"
      ></p>
    </header>
    ...
    <MatrixBody
      class="mb--4"
      v-if="
        announcement.pageFormat.pageLength &&
        $CheckA(announcement.pageFormat.body)
      "
      :matrix="announcement.pageFormat.body"
    />
    <section
      v-if="!announcement.pageFormat.pageLength"
      class="short_body richtext fs--regular"
      v-html="announcement.pageFormat.shortBody"
    ></section>
    ...
    try {
      ...
      store.commit("updatePath", [
        ...
        { title: "Announcements", route: "/" },
        {
          ...
          route: "/announcements/" + params.announcement,
        },
      ]);
