        ...
        if($CheckA(page)){
        	paths.push({title: page[0].title, route: `/publications/type/${params.type}`})
				...
				let publications = res.data.data.publications.edges.map(e => { 
					...
					const contributors = $CheckA(pub.networkRelation.associatedNetwork) ? pub.networkRelation.associatedNetwork.map(n => n.slug) : []

					const tags = $CheckA(pub.sitewideTags.edges) ? pub.sitewideTags.edges.map(ee => ee.node.slug) : []
				...
				const constructArray = (pubs, focus, setYearsAndNetwork) => {
					let filters = []
					pubs.forEach(p => {
						const f = $CheckA(p[focus].edges) ? p[focus].edges.map(e => e.node) : []
					...
					const filtered = filters.filter(({ slug }, index) => ids.indexOf(slug) === index)

					if(setYearsAndNetwork){
						const yearIds = years.map(o => o.slug)
						years = years.filter(({ slug }, index) => yearIds.indexOf(slug) === index)

						const contributorIds = contributors.map(o => o.slug)
						contributors = contributors.filter(({ slug }, index) => contributorIds.indexOf(slug) === index)
					}
		      ...
		      const artistNamesA = a.name.split(' '),
		            ...
		            artistAlphaA = (a.nameToBeAlphabetized !== null && a.nameToBeAlphabetized !== undefined && a.nameToBeAlphabetized !== '') ? a.nameToBeAlphabetized : artistLastNameA

		      const artistNamesB = b.name.split(' '),
		            artistLastNameB = artistNamesB[artistNamesB.length - 1],
		            artistAlphaB = (b.nameToBeAlphabetized !== null && b.nameToBeAlphabetized !== undefined && b.nameToBeAlphabetized !== '') ? b.nameToBeAlphabetized : artistLastNameB
