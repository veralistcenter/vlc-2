    ...
    <section>
      ...
      >
        <h2
          ...
          v-html="!year.year.includes('valid') ? year.year : '—'"
        ></h2>
...
export default {
  ...
  computed: {
        ...
        const filteredPosts = posts.filter((p) => {
          ...
          if (state.$CheckA(tags.edges)) {
            const slugs = tags.edges
              .filter((e) => state.$Check(e) && state.$Check(e.node))
              .map((e) => e.node.slug);
            const matchingSlugs = slugs.filter((s) =>
              state.activeFilters.includes(s)
            );
    ...
    byYear() {
      ...
      posts.forEach((e) => {
        const year = this.$Check(e)
          ? this.$moment(e.pageInfo.date).format("YYYY")
          : "Unsorted";
    ...
    byLetter() {
      ...
      let posts = [].concat(this.allPosts).sort((a, b) => {
        ...
        const bLetter =
          this.$Check(b) && b.title !== null ? bTitle[0].toUpperCase() : "A";
        const aLetter =
          this.$Check(a) && a.title !== null ? aTitle[0].toUpperCase() : "B";
        return aLetter.localeCompare(bLetter);
      });

      posts.forEach((e) => {
        const eTitle =
          this.$Check(e) && this.$Check(e.title)
            ? e.title.replace(/[^a-z]/gi, "")
            : "A";
    ...
    allPosts() {
      const posts = []
        .concat(this.events)
        .concat(this.additionalEvents)
        .concat(this.publications)
        .concat(this.exhibitions)
        .concat(this.announcements);
