<template>
  <main class="page pt--7 search_page">
    ...
    <section v-if="!results && isSearching" class="grid">
      <p class="fs--large">Searching...</p>
    </section>
    <section v-else-if="!results && !isSearching" class="grid">
      <p class="fs--large">No results were returned for this search.</p>
    </section>

    <section
      class="grid mb--2"
      v-if="$Check(results.networks) && $CheckA(results.networks.edges)"
    >
    ...
    <section
      class="grid mb--2"
      v-if="$Check(results.events) && $CheckA(results.events.edges)"
    >
      <h2 class="fs--large">
        {{ results.events.edges.length }} Event{{
          results.events.edges.length > 1 ? "s" : ""
        }}
      </h2>
      <GridThumbs
        class="grid--sans"
        size="Quarter"
        :posts="results.events.edges.map((e) => e.node)"
      />
    </section>

    <section
      class="grid mb--2"
      v-if="$Check(results.exhibitions) && $CheckA(results.exhibitions.edges)"
    >
      <h2 class="fs--large">
        {{ results.exhibitions.edges.length }} Exhibition{{
          results.exhibitions.edges.length > 1 ? "s" : ""
        }}
      </h2>
      <GridThumbs
        class="grid--sans"
        size="Half"
        :posts="results.exhibitions.edges.map((e) => e.node)"
      />
    </section>

    <section
      class="grid mb--2"
      v-if="$Check(results.publications) && $CheckA(results.publications.edges)"
    >
      <h2 class="fs--large">
        {{ results.publications.edges.length }} Publication{{
          results.publications.edges.length > 1 ? "s" : ""
        }}
      </h2>
      <GridThumbs
        class="grid--sans"
        size="Quarter"
        :posts="results.publications.edges.map((e) => e.node)"
      />
    </section>
    ...
    <section
      class="grid mb--2"
      v-if="
        $Check(results.announcements) && $CheckA(results.announcements.edges)
      "
    >
      <h2 class="fs--large">
        {{ results.announcements.edges.length }} Announcement{{
          results.announcements.edges.length > 1 ? "s" : ""
        }}
      </h2>
      <GridThumbs
        class="grid--sans"
        size="Quarter"
        :posts="results.announcements.edges.map((e) => e.node)"
      />
    </section>
  </main>
</template>
  ...
  async fetch() {
    ...
    try {
      ...
      const res = await this.$axios(
        this.$ReqWREST("/search?search=" + searchQuery + "&per_page=100")
      );
