<template>
  <div
    ...
    ref="page"
  >
    <button @click="skipToContent" class="skip_to" tabindex="1">
      Skip to Content
    </button>

    <transition name="fade">
      <SiteHeader
        v-if="(showMenu && $route.name === 'index') || $route.name !== 'index'"
      />
    </transition>
  ...
  computed: {
    ...
    gradients() {
      return this.settings?.pages?.edges.map((p) => {
        return {
          slug: p.node.slug,
          gradient: this.$CheckA(p.node.gradient.gradientPicker)
            ? p.node.gradient.gradientPicker[0].gradientPicker
            : false,
        };
      });
    },
    ...
    replaceFileLinks() {
      ...
      as.forEach((a) => {
        ...
        a.setAttribute(
          "href",
          link
            .replace("www.veralist", "admin.veralist")
            .replace("staging.veralist", "admin.veralist")
            .replace("://veralist", "://admin.veralist")
        );
      });
    },
    skipToContent() {
      const nodes = document.querySelectorAll(
        "main.page:not(.homepage_index) a, main.page:not(.homepage_index) button, .primary_carousel a, .primary_carousel button, .matrix_block a, .matrix_block button"
      );
    ...
    setUpUserway() {
      if (document) {
        (function (d) {
          ...
          s.setAttribute("data-account", "YgSAx9OhpA");
