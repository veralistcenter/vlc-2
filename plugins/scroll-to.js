...
Vue.prototype.$scrollToTarget = (target, ctx) => {
	...
	// cancel if cant find
	if(target.i !== true){
		if(document.querySelector(t) === null || document.querySelector(t) === undefined){ return }
	}
...
Vue.prototype.$jumpToTarget = (target, ctx) => {
	...
	if(document.querySelector('#' + target) === null || document.querySelector('#' + target) === undefined){ return }

	let targetArea = window.scrollY + document.querySelector("#" + target).getBoundingClientRect().top
