<template>
  <header class="single_header">
    <div
      class="grid"
      :class="{ 'grid--center': !$Check(post.featImage.featuredImage) }"
    >
      <section
        class="col col--1_2 mcol--full pb--1"
      ...
      >
        <figure class="header_image">
          ...
          />
          <figcaption
            style="visibility: hidden; overflow: hidden; height: 0px"
            class="mt--1_2 fs--small"
            v-html="post.featImage.featuredImage.caption"
          ></figcaption>
        </figure>
      </section>
      <section
        ...
        :class="{
          'col--1_2': $Check(post.featImage.featuredImage),
          'col--header': !$Check(post.featImage.featuredImage),
        }"
      >
        ...
        <h3
          class="genath title"
          v-if="$Check(post.pageInfo.date)"
          v-html="
            $Dated({ start: post.pageInfo.date, end: post.pageInfo.endDate })
          "
        ></h3>
        <h4
          class="genath title"
          v-html="
            $Check(post.pageInfo.timeOverride)
              ? post.pageInfo.timeOverride
              : timeRange
          "
        ></h4>
        ...
        <section
          ...
          v-html="post.pageInfo.previewInfo.description"
        ></section>
    ...
    <aside
      role="caption"
      v-if="$Check(post.featImage.featuredImage)"
      class="section_inset image_caption max--auto mt--1_2 fs--small"
    >
      <div
        v-if="$Check(post.featImage.imageCaption)"
        v-html="post.featImage.imageCaption"
      ></div>
...
export default {
  head() {
    ...
    if (this.$Check(this.post.pageInfo.previewInfo.description)) {
      ...
      description = this.$Check(this.post.pageInfo.timeOverride)
        ? this.post.pageInfo.timeOverride
        : this.$Dated({
            start: this.post.pageInfo.date,
            end: this.post.pageInfo.endDate,
          });
    }

    return this.$metatags({
      title: this.post.title,
      image: this.$Check(this.post.featImage.featuredImage)
        ? this.post.featImage.featuredImage.sourceUrl
        : undefined,
  ...
  computed: {
    types() {
      if (this.$Check(this.post.eventTypes)) {
        return this.$CheckA(this.post.eventTypes.edges)
          ? this.post.eventTypes.edges.map((e) => e.node.name).join(", ")
          : "Event";
      } else if (this.$Check(this.post.exhibitionTypes)) {
        return this.$CheckA(this.post.exhibitionTypes.edges)
          ? this.post.exhibitionTypes.edges.map((e) => e.node.name).join(", ")
          : "Exhibition";
      } else if (this.post.__typename === "Announcement") {
        return this.post.announcementTypes.edges.length > 0
          ? this.post.announcementTypes.edges.map((e) => e.node.name).join(", ")
          : "Announcement";
      }
    },
    tags() {
      if (
        this.$Check(this.post.sitewideTags) &&
        this.$CheckA(this.post.sitewideTags.edges)
      ) {
    ...
    timeRange() {
      ...
      if (this.$Check(timeEnd) && !this.$Check(timeStart)) {
        ...
        if (timeEnd.includes("pm") && timeStart.includes("pm")) {
          return `${timeStart.replace(" pm", "")}–${timeEnd.replace(
            " ",
            ""
          )} ET`;
        } else if (timeEnd.includes("am") && timeStart.includes("am")) {
          return `${timeStart.replace(" am", "")}–${timeEnd.replace(
            " ",
            ""
          )} ET`;
        } else {
