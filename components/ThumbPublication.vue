<template>
  <nuxt-link
    v-if="$Check(pub.slug)"
    class="col thumb thumb_pub mb--4"
  ...
  >
    ...
    <h2 class="genath g--small">
      ...
      <template
        v-if="
          $Check(pub.pageInfo?.previewInfo) &&
          $Check(pub.pageInfo.previewInfo.author)
        "
      >
        <br />
...
export default {
  ...
  computed: {
    ...
    description() {
      return this.$Check(this.pub.featImage) &&
        this.$Check(this.pub.featImage.subtitle)
        ? this.pub.featImage.subtitle
        : false;
    },
  },
};
