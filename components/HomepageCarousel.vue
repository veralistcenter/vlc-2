<template>
  <section class="homepage_carousel" v-if="$CheckA(gallery)">
          ...
          <div class="grid grid--sans">
            <section class="col col--3_5 mcol--full carousel_image mmt--4">
              <img
                v-if="
                  $Check(s.featImage) &&
                  $Check(s.featImage.slideshowImage) &&
                  $Check(s.featImage.slideshowImage.sourceUrl)
                "
              ...
              <img
                v-else-if="
                  $Check(s.featImage) &&
                  $Check(s.featImage.featuredImage) &&
                  $Check(s.featImage.featuredImage.sourceUrl)
                "
            ...
            <section
              ...
              :class="{
                'pt--1': gallerySlides.length == 1,
                'pt--4 mpt--2': gallerySlides.length > 1,
              }"
            >
              <nuxt-link :to="generatePath(s)">
                <h1
                  class="fs--small caps mb--1_2"
                  v-html="generateType(s)"
                ></h1>
                <h2 class="genath title" v-html="s.title"></h2>
                <h3
                  class="genath title"
                  v-html="
                    $Check(s.pageInfo) && $Check(s.pageInfo.timeOverride)
                      ? s.pageInfo.timeOverride
                      : $Dated({
                          start: s.pageInfo.date,
                          end: s.pageInfo.endDate,
                        })
                  "
                ></h3>
              </nuxt-link>
              ...
              <p
                ...
                v-if="
                  $Check(s.pageInfo.previewInfo) &&
                  $Check(s.pageInfo.previewInfo.primaryDescription)
                "
    ...
    <nav
      v-if="$CheckA(gallerySlides) && gallerySlides.length > 1"
      class="homepage_carousel_nav grid grid--sans"
    >
      <aside class="col col--2_3 mcol--1_2">
        <button
          ...
          class="hc_nav_button_dots mr--1_2"
        ></button>
      </aside>
      <section class="col col--1_3 mcol--1_2 mcol--end col--end right">
        <p>
          <button
            ...
            class="hc_nav_buttons mr--1_2"
          >
            <img src="/previous-inline.svg" />
          </button>
          <button @click="$refs.myVueperSlides.next()" class="hc_nav_buttons">
...
export default {
  ...
  computed: {
    gallerySlides() {
      return [].concat(
        this.gallery.filter((g) => this.$Check(g) && this.$Check(g.pageInfo))
      );
    },
