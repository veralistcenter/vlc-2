    ...
    <section class="grid">
      ...
      <aside class="col col--1_2 mcol--full col--end pub_info mb--1">
        <section
          ...
          v-html="previewInfo.primaryDescription"
        ></section>
        ...
        <section class="mt--1" v-if="$Check(previewInfo.button.buttonLink)">
          <a
            ...
            v-html="previewInfo.button.buttonName"
          ></a>
        </section>
        ...
        <section
          ...
          v-html="previewInfo.secondaryDescription"
        ></section>
      </aside>
    </section>
  </header>
...
export default {
  head() {
    const image = this.pub.featImage.featuredImage
      ? this.pub.featImage.featuredImage.sourceUrl
      : undefined;

    const description = this.$Check(this.previewInfo.primaryDescription)
      ? this.previewInfo.primaryDescription
      : undefined;
  ...
  computed: {
    ...
    format() {
      return this.$CheckA(this.pub.publicationFormats.edges)
        ? this.pub.publicationFormats.edges.map((e) => e.node.name).join(", ")
        : "Publication";
    },
    tags() {
      if (
        this.$Check(this.pub.sitewideTags) &&
        this.$CheckA(this.pub.sitewideTags.edges)
      ) {
...
.pub_info {
  margin-top: calc(var(--fs-g) - var(--fs-r) + 0.15rem);
}
</style>
