  ...
  methods: {
    async fetchArchivedPosts() {
      try {
        ...
        const events = d.events.edges
          .map((e) => e.node)
          .filter((e) => e.archiveSelection.selectedArchiveEvent == true);
        const exhibitions = d.exhibitions.edges
          .map((e) => e.node)
          .filter(
            (e) =>
              e.exhibitionArchiveSelection.selectedArchiveExhibition == true
          );
        const publications = d.publications.edges
          .map((e) => e.node)
          .filter(
            (e) =>
              e.archiveSelectionPublication.selectedArchivePublication == true
          );
