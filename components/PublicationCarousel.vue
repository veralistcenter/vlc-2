<template>
  <section class="pub_carousel">
    <nav class="pub_nav grid">
      <section class="col col--2_3 pc_nav_buttons mb--1_2">
        <button
          ...
          class="pc_nav_button_dots mr--1_2"
        ></button>
      </section>
      <section class="col col--1_3 col--end right">
        <p>
          <button
            ...
            class="pc_nav_buttons mr--1_2"
          >
            <img src="/previous-inline.svg" />
          </button>
          <button @click="$refs.myVueperSlides.next()" class="pc_nav_buttons">
    ...
    <vueper-slides
      ...
      @slide="changeActive"
    >
      <vueper-slide v-for="(s, i) in slides" :key="'pub_' + i">
        <template #content>
          <div class="pub_feat_slide">
            <figure v-if="$Check(s.slideImage.sourceUrl)">
              ...
              />
              <figcaption
                v-if="$Check(s.slideImage.caption)"
                v-html="s.slideImage.caption"
              ></figcaption>
            </figure>
          </div>
        </template>
