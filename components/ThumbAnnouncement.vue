<template>
  ...
  >
    ...
    <img
      v-if="
        $Check(announcement.featImage) &&
        $Check(announcement.featImage.featuredImage)
      "
    ...
    <h3
      ...
      :class="
        $IsCurrent({
          start: announcement.pageInfo.date,
          end: announcement.pageInfo.endDate,
        })
          ? 'current_event'
          : 'past_event'
      "
      v-html="
        $Check(announcement.pageInfo.timeOverride)
          ? announcement.pageInfo.timeOverride
          : $Dated({
              start: announcement.pageInfo.date,
              end: announcement.pageInfo.endDate,
            })
...
export default {
  ...
  computed: {
    announcementTypes() {
      return this.announcement?.announcementTypes &&
        this.announcement.announcementTypes.edges.length > 0
        ? this.announcement.announcementTypes.edges
            .map((e) => e.node.name)
            .join(", ")
        : "Announcement";
    },
