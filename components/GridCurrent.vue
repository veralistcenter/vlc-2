      ...
      const isCurrent = (pageInfo) => {
        if (this.$Check(pageInfo.endDate)) {
          return this.$moment().isAfter(this.$moment(pageInfo.date)) &&
            (this.$moment().isBefore(this.$moment(pageInfo.endDate)) ||
              this.$moment(pageInfo.endDate).isSame(this.$moment(), "day"))
            ? true
            : false;
        } else {
          return this.$moment(pageInfo.date).isSame(this.$moment(), "day")
            ? true
            : false;
        }
      };
